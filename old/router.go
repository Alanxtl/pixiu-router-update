...
func (rm *RouterCoordinator) Route(hc *stdHttp.Request) (*model.RouteAction, error) {
	rm.rw.RLock()
	defer rm.rw.RUnlock()
...
func (rm *RouterCoordinator) OnAddRouter(r *model.Router) {
	...
	if r.Match.Methods == nil {
		r.Match.Methods = []string{"GET", "PUT", "DELETE", "POST", "OPTIONS"}
	}
...
func (rm *RouterCoordinator) OnDeleteRouter(r *model.Router) {
	...
	if r.Match.Methods == nil {
		r.Match.Methods = []string{"GET", "PUT", "DELETE", "POST"}
	}
	isPrefix := r.Match.Prefix != ""
	for _, method := range r.Match.Methods {
		var key string
